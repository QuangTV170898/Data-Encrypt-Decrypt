...
static void rotate(unsigned char *word);
...
static unsigned char getRconValue(unsigned char num);

// Implementation: Key Schedule Core
static void core(unsigned char *word, int iteration);

// Implementation: Key Expansion



static void expandKey(unsigned char *expandedKey, unsigned char *key, enum keySize, size_t expandedKeySize);
...
static void shiftRow(unsigned char *state, unsigned char nbr);
...
static void aes_round(unsigned char *state, unsigned char *roundKey);
// Implementation: the main AES body
static void createRoundKey(unsigned char *expandedKey, unsigned char *roundKey);
static void aes_main(unsigned char *state, unsigned char *expandedKey, int nbrRounds);
// Implementation: AES encryption
static char aes_encrypt(unsigned char *input, unsigned char *output, unsigned char *key, enum keySize size);
// AES Decryption
static void invSubBytes(unsigned char *state);
static void invShiftRows(unsigned char *state);
static void invShiftRow(unsigned char *state, unsigned char nbr);
static void invMixColumns(unsigned char *state);
static void invMixColumn(unsigned char *column);
static void aes_invRound(unsigned char *state, unsigned char *roundKey);
static void aes_invMain(unsigned char *state, unsigned char *expandedKey, int nbrRounds);
static char aes_decrypt(unsigned char *input, unsigned char *output, unsigned char *key, enum keySize size);
...
char* aes_128_encrypt_full(unsigned char *input){
    char cipher_str[128];
    int num_block_16byte = strlen(input)/16;
    num_block_16byte = (strlen(input)%16)?num_block_16byte+1:num_block_16byte;
    char **plain_str = (char **)calloc(num_block_16byte, sizeof(char*));
    for(int i =0; i<num_block_16byte;i++){
        *(plain_str+i) = (char *)calloc(16, sizeof(char));
        memcpy(*(plain_str+i) , input + i*16, 16);
        aes_encrypt(*(plain_str+i),*(plain_str+i),key,size);
    }
    char *output = (char*)calloc(16*num_block_16byte, sizeof(char));
    for(int i = 0; i<num_block_16byte;i++){
        strcat(output, *(plain_str+i));
        free(*(plain_str+i));
    }
    free(plain_str);
    return output;
}
char* aes_128_decrypt_full(unsigned char *input){
    char cipher_str[128];
    int num_block_16byte = strlen(input)/16;
    num_block_16byte = (strlen(input)%16)?num_block_16byte+1:num_block_16byte;
    char **plain_str = (char **)calloc(num_block_16byte, sizeof(char*));
    for(int i =0; i<num_block_16byte;i++){
        *(plain_str+i) = (char *)calloc(16, sizeof(char));
        memcpy(*(plain_str+i) , input + i*16, 16);
        aes_decrypt(*(plain_str+i),*(plain_str+i),key,size);
    }
    char *output = (char*)calloc(16*num_block_16byte, sizeof(char));
    for(int i = 0; i<num_block_16byte;i++){
        strcat(output, *(plain_str+i));
